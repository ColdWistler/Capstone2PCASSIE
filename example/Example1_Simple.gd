...
const SAVE_PATH_BASE = "user://dqn_weights"
...
func _best_run_path() -> String:
	if seed != 0:
		return "user://dqn_seed_%d_run_%d.save" % [seed, run_id]
	return "user://dqn_run_%d.save" % run_id
...
func _find_best_run() -> String:
	...
	var fn = dir.get_next()
	while fn != "":
		if fn.ends_with(".save") and (fn.begins_with("dqn_run_") or fn.begins_with("dqn_seed_")):
			var full = "user://" + fn
